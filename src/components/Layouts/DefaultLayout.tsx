...
const DefaultLayout = ({ children }: PropsWithChildren) => {
    ...
    useEffect(() => {
        const checkAuth = async () => {
            try {
                await dispatch(fetch()).unwrap(); // calls /me and gets user
                // User fetched successfully, no redirect needed
            } catch (err) {
                // Fetch failed - probably no valid token cookie or session expired
                navigate('/auth/login');
            }
        };
        checkAuth();
